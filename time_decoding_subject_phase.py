...
X = np.vstack([
    np.mean(np.angle(data_ctl_left), axis=2),
    np.mean(np.angle(data_ctl_right), axis=2),
    np.mean(np.angle(data_ent_left), axis=2),
    np.mean(np.angle(data_ent_right), axis=2)
])
y = np.concatenate([
