...
events = mne.event.merge_events(events, [16, 32, 64, 128],
                                99, replace_events=True)
...
epochs = mne.Epochs(raw, events, event_id, tmin, tmax, picks=picks,
                    baseline=(None, -1.2), reject=reject_params,
                    add_eeg_ref=True,
                    preload=False)
epochs.drop_bad(reject_params)

fig = epochs.plot_drop_log(subject=subject, show=False)
fig.savefig(epochs_folder + "pics/%s_target_drop_log.png" % subject)

epochs.save(epochs_folder + "%s_target-epo.fif" % subject)
